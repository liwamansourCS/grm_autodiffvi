...
from torch.distributions import Gamma, HalfNormal, Normal
...
def compute_mse(y, x, model_params, advi_mode):
    ...
    if advi_mode == "fullrank":
        betas = sample["params"][:, :100]
        sig = sample["params"][:, 100:101].exp()
...
def log_prior_plus_logabsdet_J(beta, sig, alpha):
    # log prior sig + log jacobian
    lp_log_sig = (Gamma(1, 1).log_prob(sig.exp()) + sig).sum()

    # log prior alpha + log jacobian
    lp_log_alpha = (Gamma(1, 1).log_prob(alpha.exp()) + alpha).sum()

    # log prior for beta, evaluated at sampled values for beta
    lp_b = Normal(0, sig.exp()/alpha.exp().sqrt()).log_prob(beta).sum()


    # return lp_b + lp_log_sig

    return lp_b + lp_log_sig + lp_log_alpha
...
def elbo(
    ...
    if advi_mode == "meanfield":
        ...
        beta = sample["params"][:100]
        sig = sample["params"][100:101]
        alpha = sample["params"][101:]

    out += loglike(y_train, x_train, beta, sig, full_data_size)
    out += log_prior_plus_logabsdet_J(beta, sig, alpha)
